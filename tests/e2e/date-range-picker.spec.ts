...
async function openPopup(page: any) {
...
test.describe('DateRangePicker desktop', () => {
  ...
  test('dr-next scrolls forward', async ({ page }) => {
    ...
    const scrollBefore = await scrollEl.evaluate((el: HTMLElement) => el.scrollLeft)
    await page.click('[data-testid="dr-next"]')
    // Wait for scroll animation
    await page.waitForTimeout(500)
    const scrollAfter = await scrollEl.evaluate((el: HTMLElement) => el.scrollLeft)
...
test.describe('DateRangePicker mobile', () => {
  ...
  test('dr-weekdays stays sticky during scroll', async ({ page }) => {
    ...
    // Scroll the calendar
    await page.locator('[data-testid="dr-scroll"]').evaluate((el: HTMLElement) => {
      el.scrollTop = 300
    })
